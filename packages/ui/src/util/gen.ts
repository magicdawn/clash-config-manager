...
export default async function genConfig({ forceUpdate = false }: { forceUpdate?: boolean } = {}) {
  ...
  const autoGeneratedProxyGroupsForSubscribe = subscribeTragets
      ...
      return [
        {
          ...
          proxies: [`${subscribeName}-最快`, `${subscribeName}-可用`, `${subscribeName}-手选`],
        },
        {
          name: `${subscribeName}-最快`,
