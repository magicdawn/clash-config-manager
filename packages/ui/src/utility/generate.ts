...
export async function generateConfig({ forceUpdate = false }: { forceUpdate?: boolean } = {}) {
  ...
  let proxyGroupsWithFilter = proxyGroups.filter((item) => item.filter)
  ...
  middleProxyGroups.forEach((item) => {
    delete item.middle // clean up
    item.proxies = [
      'DIRECT',
      ...autoGeneratedProxyGroupsForSubscribe.map((proxyGroup) => proxyGroup.name),
      ...proxyGroupsWithFilter.map((proxyGroup) => proxyGroup.name),
    ]
  })
  ...
  const prependProxyGroups = [
    {
      ...
      type: ProxyGroupType.Select,
      proxies: [
        ...middleProxyGroups.map((proxyGroup) => proxyGroup.name),
        ...autoGeneratedProxyGroupsForSubscribe.map((proxyGroup) => proxyGroup.name),
        ...proxyGroupsWithFilter.map((proxyGroup) => proxyGroup.name),
      ],
    },
    ...
    ...proxyGroupsWithFilter,
  ]

  // 去除 prepend proxy-groups
  const prependProxyGroupsNames = new Set(prependProxyGroups.map((pg) => pg.name))
  proxyGroups = proxyGroups.filter((proxyGroup) => !proxyGroup.filter && !prependProxyGroupsNames.has(proxyGroup.name))
  ...
  const existingProxyGroupNames = new Set(proxyGroups.map((item) => item.name))
  const reservedTargets = new Set(['DIRECT', 'REJECT', 'no-resolve'])
  const toAddGroups = new Set<string>()

  for (const line of config.rules || []) {
    const use = line.split(',').at(-1)
    if (!use) continue
    if (reservedTargets.has(use)) continue
    if (existingProxyGroupNames.has(use)) continue
    toAddGroups.add(use)
  }
